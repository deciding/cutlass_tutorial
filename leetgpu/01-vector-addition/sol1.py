...
def elementwise_add_kernel(
    ...
    # NOTE: thrCrd always treat input as a index and output a 2d index
    for i in range(0, cute.size(frgPred), 1):
        val = cute.elem_less(thrCrd[i], shape) # e.g. (0, 996) < (1, 2048)
        frgPred[i] = val
...
def solve(A: cute.Tensor, B: cute.Tensor, C: cute.Tensor, N: cute.Uint32):
    ...
    layout = cute.make_layout((1, N))
    # change the layout, make another cute.Tensor
