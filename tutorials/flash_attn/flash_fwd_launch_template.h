...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    //>;
    using CollectiveMainloop = flash::CollectiveMainloopFwdSm90<kStages, ClusterShape, TileShape_MNK, kHeadDimV, Element, float, cutlass::arch::Sm90, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV, HasQv, MmaPV_is_RS, IntraWGOverlap, PackGQA, Split, V_colmajor>;
...
void run_mha_fwd_(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    printf("  kHeadDimV: %d\n", kHeadDimV);
    printf("  Split: %s\n", Split ? "true" : "false");
    printf("  PagedKVNonTMA: %s\n", PagedKVNonTMA ? "true" : "false");
    printf("  Has_softcap: %s\n", Has_softcap ? "true" : "false");
    printf("  PackGQA: %s\n", PackGQA ? "true" : "false");
